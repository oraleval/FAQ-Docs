...
>> 返回结果字段：audioCheck
>> 字典类型，包含volume、clipping、noise、cut,tooShort,emptyAudio，判断的优先级emptyAudio>tooShort>其他项，如果emptyAudio的值为true，则说明音频为空，可以直接提示用户未检测到录音等；如果tooShort为true,则提示用户重新录音
